...
(async () => {
  ...
  try {
    ...
    const launcherBeforeDrag = await launcher.boundingBox();
    await page.mouse.move(launcherBeforeDrag.x + 20, launcherBeforeDrag.y + 20);
    await page.mouse.down();
    await page.mouse.move(launcherBeforeDrag.x - 48, launcherBeforeDrag.y - 38);
    await page.mouse.up();
    const launcherAfterDrag = await launcher.boundingBox();
    assert.ok(launcherAfterDrag.x < launcherBeforeDrag.x, "展开状态的常驻图标应支持拖动");
    assert.equal(typeof (await page.evaluate(() => globalThis.__gmValues.launcherPosition))?.left, "number", "小图标位置应持久化");
    const launcherCollapse = page.locator("#gpt-formula-copy-control #launcher-collapse");
    await launcherCollapse.click();
    assert.equal(await page.locator("#gpt-formula-copy-control #launcher-shell").getAttribute("data-collapsed"), "true");
    assert.doesNotMatch(await launcher.innerText(), /公式复制/, "收起状态只保留 Logo 入口");
    await page.waitForTimeout(220);
    const collapsedBeforeDrag = await launcher.boundingBox();
    assert.equal(await launcher.evaluate((element) => getComputedStyle(element).cursor), "grab", "Logo-only 状态应明确提示可拖动");
    await page.mouse.move(collapsedBeforeDrag.x + 17, collapsedBeforeDrag.y + 34);
    await page.mouse.down();
    await page.mouse.move(collapsedBeforeDrag.x - 30, collapsedBeforeDrag.y - 18);
    await page.mouse.up();
    const collapsedAfterDrag = await launcher.boundingBox();
    assert.ok(collapsedAfterDrag.x < collapsedBeforeDrag.x, "仅 Logo 状态也应支持拖动");
    assert.equal(typeof (await page.evaluate(() => globalThis.__gmValues.launcherPosition))?.top, "number", "仅 Logo 状态拖动后的位置应持久化");
    await launcherCollapse.click();
    assert.equal(await page.locator("#gpt-formula-copy-control #launcher-shell").getAttribute("data-collapsed"), "false");
    ...
    assert.match(await updateNotice.innerText(), /5\.0\.1/);
    ...
    assert.equal(await page.evaluate(() => globalThis.__GPT_LATEX_COPY_API__.getStatus().version), "5.0.0");
